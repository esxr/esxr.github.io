...
We learnt that the `chat` endpoint is a streaming endpoint. We read the input as an **event-stream**, and we'll format the dummy output to the **Newline delimited JSON** (`application/x-ndjson`) format.
